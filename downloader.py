...
# [FIX 1] Token cache variable to prevent 3-second delays on every request
...
async def _process_single_file_metadata(item, share_id, uk, share_session, existing_files, action, wait_for_transcoding, bdstoken_val, quality=None):
    ...
    # Step A: Check if the file already exists (Fast Storage Bypass)
    my_fs_id = ""
    my_file_path = ""
    if filename in existing_files and existing_files[filename]["size"] == size_bytes:
        ...
        if transfer_res.get("errno") == 2:
            ...
            create_payload = { "path": ROOT_PATH, "isdir": "1", "size": "0", "block_list": "[]", "method": "post" }
            try:
    ...
    # [FIX 3] Fully bypassed slow M3U8 waiting logic for faster Dlink/Stream proxy
...
async def _resolve_link(link, action="d", wait_for_transcoding=False, quality=None):
    ...
    import time
    # [FIX 1 Continued] Token Cache: Only refresh if > 1 hour old
    ...
    except ValueError as e: return {"errno": -3, "error": f"Invalid share link: {e}"}
        
    list_url = f"{BASE_PUBLIC}/share/list?app_id=250528&shorturl={surl}&root=1&order=name&desc=0&showempty=0&web=1&page=1&num=100"
    try:
    ...
    if share_data.get("errno") != 0:
        return {"errno": share_data.get("errno"), "error": f"Share link is invalid or expired."}
    ...
    if action != "l":
        ...
        try:
            # [FIX 2] Reduced num to 500 so it loads very fast but still prevents Storage Full 
            r_list = await get_session().get(
                f"{BASE_API}/api/list?{qp()}&dir={encoded_dir}&order=time&desc=1&showempty=0&page=1&num=500&bdstoken={BDSTOKEN}",
                timeout=10.0
            )
            ...
            print(f"[TeraBridge][WARN] Failed to list existing files: {e}", flush=True)

    results = []
    for item in files_list:
        results.append(await _process_single_file_metadata(
            item, share_id, uk, share_session, existing_files, action, wait_for_transcoding, BDSTOKEN, quality
        ))
    ...
    if action != "l":
        ...
        if fs_ids_to_resolve:
            ...
            for chunk in fs_id_chunks:
                ...
                # Added thumb=1 for instant thumbnail resolving
            ...
            for r in results:
                if r.get("fs_id") and not r.get("error"):
                    my_fs_id = r["fs_id"]
                    if my_fs_id in dlink_map: r["dlink"] = dlink_map[my_fs_id]
                    if my_fs_id in thumb_map: r["thumbnails"] = thumb_map[my_fs_id]
...
async def download_file(dlink, filename):
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=120.0) as dl_client:
            async with dl_client.stream("GET", dlink, headers={"User-Agent": UA, "Referer": BASE_API + "/"}, cookies=COOKIES_DICT) as dr:
